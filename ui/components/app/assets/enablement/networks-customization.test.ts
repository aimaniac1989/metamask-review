...
import {
  ...
  filterExcludedTokenManagementAssets,
...
describe('networks-customization', () => {
  ...
  describe('filterExcludedTokenManagementAssets', () => {
    it('filters by hex chainId + address', () => {
      ...
      expect(filterExcludedTokenManagementAssets(assets)).toStrictEqual([
        { chainId: ARC, address: OTHER_TOKEN },
        { chainId: '0x1', address: ARC_USDC_ERC20_TOKEN_ADDRESS },
      ]);
    });
    ...
    it('passes through non-EVM, unparseable, and chainless assets', () => {
      ...
      expect(filterExcludedTokenManagementAssets(assets)).toStrictEqual(assets);
    });
  });
