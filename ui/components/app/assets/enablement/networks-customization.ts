...
const EXCLUDED_ASSET_IDS = new Set(
  Object.entries(EXCLUDED_ASSETS_FROM_ASSET_LIST).map(
    ([hexChainId, address]) =>
      `eip155:${parseInt(hexChainId, 16)}/erc20:${address}`,
  ),
);
 ...
 * @param assets
 */
export function filterExcludedTokenManagementAssets<
  AssetGeneric extends AssetLike,
>(assets: AssetGeneric[]): AssetGeneric[] {
